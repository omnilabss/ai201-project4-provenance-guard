...
**High-confidence AI — but note: signals disagreed (confidence: 0.652 → uncertain)**
...
```json
{
  ...
  "signal_scores": {
    "llm_score": 0.91,
    "stylometric_score": 0.4624
  },
  "label": "◐ Attribution Uncertain — Our analysis could not determine with confidence whether this content was written by a person or generated by AI. Confidence: 65% AI indicators. Authors may appeal if this label is inaccurate."
}
```

> **Why uncertain instead of likely\_ai?** The LLM signal strongly flagged this text (0.91), but the stylometric signal disagreed (0.46). The gap (0.45 > 0.40 threshold) triggered disagreement dampening, pulling the combined score from 0.754 down to 0.652. This means the final confidence still lands just below the 0.70 threshold for `likely_ai`. This is the system working as designed: when signals conflict, it acknowledges uncertainty rather than over-committing.

**Low-confidence human (confidence: 0.269)**
...
```json
{
  ...
  "signal_scores": {
    "llm_score": 0.21,
    "stylometric_score": 0.3788
  },
  "label": "✓ Likely Human-Written — Our analysis found strong indicators that this content was written by a person. Confidence: 73% human. No action required."
}
```

The **0.383-point gap** between these two examples (0.269 vs 0.652) demonstrates that the scoring produces **meaningful variation** across clearly different inputs. Adding the formal-human-writing test case (confidence: 0.7177, `likely_ai`) extends the full observed range to **0.269 – 0.7177 across 4 test inputs**.

**Full 4-input test results (all real Groq API scores):**

| Input Type | LLM Score | Stylo Score | Confidence | Attribution |
|---|---|---|---|---|
| Clearly AI-generated | 0.91 | 0.4624 | 0.652 | `uncertain` \* |
| Clearly human (casual) | 0.21 | 0.3788 | 0.2691 | `likely_human` |
| Formal human writing | 0.85 | 0.4719 | 0.7177 | `likely_ai` |
| Lightly edited AI | 0.42 | 0.4529 | 0.4315 | `uncertain` |

\* Disagreement dampening applied (LLM–stylo gap = 0.45 > 0.40 threshold).
...
**Sample log entries (from GET /log after running `python3 run_all_tests.py`):**

```json
[
  {
    "id": 4,
    "content_id": "0f097426-2fd0-4a37-aaf4-c9f99edfbb07",
    "creator_id": "test-user-4",
    "timestamp": "2026-06-29T05:32:49.391730+00:00",
    "attribution": "uncertain",
    "confidence": 0.4315,
    "llm_score": 0.42,
    "stylometric_score": 0.4529,
    "status": "classified",
  ...
  {
    "id": 3,
    "content_id": "1afc4353-0af2-44fc-aac5-fb16989a72c2",
    "creator_id": "test-user-3",
    "timestamp": "2026-06-29T05:32:48.881131+00:00",
    "attribution": "likely_ai",
    "confidence": 0.7177,
    "llm_score": 0.85,
    "stylometric_score": 0.4719,
    "status": "classified",
    "appeal_reasoning": null,
    "appeal_timestamp": null
  },
  {
    "id": 2,
    "content_id": "96bdbb90-00f5-48a5-9f66-5eb0ce4c7a79",
    "creator_id": "test-user-2",
    "timestamp": "2026-06-29T05:32:48.511754+00:00",
    "attribution": "likely_human",
    "confidence": 0.2691,
    "llm_score": 0.21,
    "stylometric_score": 0.3788,
  ...
  {
    "id": 1,
    "content_id": "1d523553-774a-421a-bc17-18a66ec96e49",
    "creator_id": "test-user-1",
    "timestamp": "2026-06-29T05:32:48.343457+00:00",
    "attribution": "uncertain",
    "confidence": 0.652,
    "llm_score": 0.91,
    "stylometric_score": 0.4624,
    ...
    "appeal_timestamp": "2026-06-29T05:32:49.395788+00:00"
  }
]
```

All 4 entries above are **real outputs from the Groq llama-3.3-70b-versatile API**. Entry #1 has been appealed (status `under_review`), demonstrating the full lifecycle. Entry #3 (formal human writing about monetary policy) was flagged as `likely_ai` — this is the known false-positive limitation documented in the Known Limitations section.
